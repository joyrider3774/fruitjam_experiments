...
void setup() {
    ...
    Serial.println("\n╔═══════════════════════════════════════╗");
    Serial.println("║   I2STONES MUSICAL DEMO - FruitJam   ║");
    Serial.println("╚═══════════════════════════════════════╝\n");
    
    if (!setupI2SAudio(44100,AUDIO_OUT_BOTH, 4096) ) {
        Serial.println("❌ Failed to initialize I2S audio!");
        while (1) delay(100);
    }
    
    Serial.println("✓ I2S Audio initialized!");
    Serial.print("  Sample rate: "); Serial.print(getSampleRate()); Serial.println(" Hz");
    Serial.print("  Channels: "); Serial.println(getMaxChannels());
...
void laserShot() {
    ...
    playTone(1200, 200, 0.05);
    delay(50);
    playTone(800, 200, 0.05);
    delay(50);
...
void explosion() {
    ...
    for (int i = 0; i < 10; i++) {
        playTone(random(100, 500), 150, 0.05);
    }
}

void coin() {
    Serial.println("🔊 Coin!");
    playTone(NOTE_B5, 200, 0.1);
    delay(100);
    playTone(NOTE_E6, 200, 0.3);
}
...
void playScale() {
    ...
    for (int i = 0; i < 8; i++) {
        playTone(scale[i], 200, 0.25);
        delay(280);
    }
}

void playChord(float note1, float note2, float note3, float duration) {
    playTone(note1, 150, duration);
    playTone(note2, 150, duration);
    playTone(note3, 150, duration);
}
...
void playMaryHadALittleLamb() {
    ...
    for (int i = 0; i < sizeof(melody) / sizeof(Note); i++) {
        playTone(melody[i].freq, 200, melody[i].duration / 1000.0);
...
void playTwinkle() {
    ...
    for (int i = 0; i < sizeof(melody) / sizeof(Note); i++) {
        playTone(melody[i].freq, 180, melody[i].duration / 1000.0);
...
void multiChannelDemo() {
    ...
    playTone(NOTE_C4, 150, 2.0, 0.0);   // Root
    playTone(NOTE_E4, 150, 2.0, 0.2);   // Third
    playTone(NOTE_G4, 150, 2.0, 0.4);   // Fifth
    playTone(NOTE_B4, 150, 2.0, 0.6);   // Seventh
